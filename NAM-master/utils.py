...
NAMParams = collections.namedtuple('NAMParams', 'nz ngf ndf mu sd force_l2')
NAMParams.__new__.__defaults__ = (None, None, None, None, None, None)
...
def encoderVAE(sz, nz, ndf, nc):
    if sz == 32:
        net = unconditional._encoder32_VAE(nz, ndf, nc)
    elif sz == 64:
        net = unconditional._encoder64_VAE(nz, ndf, nc) 
    net.apply(unconditional.weights_init)
    return net

def encoderWAE(sz, nz, ndf, nc):
    if sz == 32:
        net = unconditional._encoder32_WAE(nz, ndf, nc)
    elif sz == 64:
        net = unconditional._encoder64_WAE(nz, ndf, nc) 
    net.apply(unconditional.weights_init)
    return net
